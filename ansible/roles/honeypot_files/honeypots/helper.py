...
class postgres_class():
    ...
    def create_tables(self):
        for x in self.mapped_tables:
            self.cur.execute(
                sql.SQL('CREATE TABLE IF NOT EXISTS {} (id SERIAL NOT NULL,date timestamp with time zone DEFAULT now(),data json)').format(
                    sql.Identifier(x + '_table')))

    def insert_into_data_safe(self, table, obj):
        with suppress(Exception):
            self.cur.execute(sql.SQL('INSERT INTO {} (id,date, data) VALUES (DEFAULT ,now(), %s)').format(
                sql.Identifier(table + '_table')), [obj])
